...
bool LinearSolver<Tag>::solve(const IMatrix& A, const IVector& b, IVector& x)
{
  ...
  SolverStatSentry<KernelSolver> sentry(m_solver.get(),SolverStater::ePrepare);
  ...
  SolverStatSentry<KernelSolver> sentry2(m_solver.get(),SolverStater::eSolve);
...
{
  SolverStatSentry<KernelSolver> sentry(m_solver.get(),SolverStater::eInit);
  m_solver->init();
}
